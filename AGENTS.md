...
- **Config.** Keys are flat: `merged_config()` does a light flat parse, so a nested YAML section arrives as a string and `.get()` on it raises — which silently disabled the daily budget cap. It must also never raise: the framework does not merge `default_config.yaml`, so a wiped `config.json` must still resolve the full YAML defaults, and a failing registry read is treated as empty rather than bubbled.
...
- **Safety.** Secrets never enter the plugin tree: the env file holds `${VAR}` references, values live in the project's `usr/.env`, and dry-run output exposes key names and booleans only. Preserve auth and CSRF — the hub-status route is the one deliberate exception, being read-only, non-sensitive, public data.
